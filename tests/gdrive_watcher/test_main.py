...
class TestGDriveWatcher:

    @pytest.fixture(autouse=True)
    def _setup(self, module_patch, flags_mock, games_config_mock):
        ...
        def mock_daemon_init(_self, service_name, requires_auth):
            _self.__service_name = service_name
            _self.__requires_auth = requires_auth
            _self._Daemon__interval = 5.0  # Set private name-mangled attribute directly
            _self._logger = MagicMock()
    ...
    def test_get_changes_updates_token_and_extracts_ids(self, gdrive_mock):
        """
        ...
        changes_response = self.create_mock_changes_response([
            {
    ...
    def test_get_changes_handles_removed_files_hack(self, gdrive_mock, app_context_mock):
        """
    ...
    def test_work_sends_refresh_for_all_relevant_changes(self, gdrive_mock, app_context_mock, games_config_mock,
        ...
        gdrive_mock.get_changes.return_value = self.create_mock_changes_response([], new_token="T1")
    ...
    def test_work_sends_no_refresh_if_no_relevant_changes(self, ui_socket_mock, app_context_mock, gdrive_mock):
        """
        ...
        gdrive_mock.get_changes.return_value = self.create_mock_changes_response([], new_token="T2")
        watcher._GDriveWatcher__get_changes = MagicMock(return_value=(modified_files, affected_directories))
