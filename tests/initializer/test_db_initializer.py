...
@pytest.fixture
def migration_exists_mock(mocker: MockerFixture):
...
@pytest.fixture
def update_schema_version_mock(mocker: MockerFixture):
    from savegem.initializer.db_initializer import DatabaseInitializer
...
def test_run_initializes_and_migrates(mocker, db_mock):
    """
...
def test_initialize_creates_schema_version_table(db_mock):
    """
...
def test_migration_exists_checks_database(mocker: MockerFixture, db_mock, file_name, expected):
    """
...
def test_update_schema_version_inserts_correct_data(db_mock, datetime_mock):
    """
    ...
    db_mock.execute.assert_called_once_with(
        pytest.approx(f"""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, abs=True),
        ('v2025_10_12_2205__Create_setup_tables', expected_version, expected_description, mock_now, 1)
    )


def test_update_schema_version_raises_on_invalid_name(db_mock):
    """
...
def test_migrate_skips_when_last_migration_exists(db_mock, listdir_mock, migration_exists_mock,
...
def test_migrate_applies_new_migrations_and_updates_schema(module_patch, read_file_mock, db_mock, listdir_mock,
                                                           migration_exists_mock, update_schema_version_mock):
    """
