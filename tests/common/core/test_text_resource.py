...
def _setup(mocker: MockerFixture, db_table_mock):
...
def test_should_not_read_file_again_if_locale_same(db_mock):
...
def test_should_read_file_if_locale_changed(db_mock):
