...
class TestUploader:
    ...
    def _setup(self, app_context_mock, user_config_mock, path_exists_mock, resolve_temp_file_mock):
    ...
    @pytest.fixture
    def _uploader(self, _mock_subscriber):
        """
    ...
    def test_upload_success(self, path_exists_mock, resolve_temp_file_mock, makedirs_mock, copy_mock, make_archive_mock,
                            gdrive_mock, datetime_mock, _uploader, _mock_game, _mock_subscriber):
        """
        ...
        # ACT
        _uploader.upload(_mock_game)

        # ASSERT - Execution Flow (5 stages + DoneEvent)

        # 1. Directory Existence Check
        path_exists_mock.assert_called_with(_mock_game.local_path)
        ...
        expected_copy_calls = [
            call(_mock_game.file_list[0], target_archive_base),
            call(_mock_game.file_list[1], target_archive_base)
        ]
        ...
        _mock_game.meta.local.calculate_checksum.assert_called_once()
        assert _mock_game.meta.local.checksum == "NEW_CHECKSUM"
        assert _mock_game.meta.local.owner == PlayerTestData.FirstPlayerName
        assert _mock_game.meta.local.created_time == mock_now.isoformat()
        # Check metadata file copy (the third call to mock_copy)
        assert copy_mock.call_args_list[2] == call(_mock_game.metadata_file_path, target_archive_base)
        ...
        assert upload_args[1] == _mock_game.drive_directory  # drive_directory
        ...
        progress_calls = [c for c in _mock_subscriber.call_args_list if not isinstance(c[0][0], DoneEvent)]
        assert len(progress_calls) == 5  # 0% + 6 full stage completions -1 since GDrive.upload_file is being mocked.


    def test_upload_saves_directory_missing(self, path_exists_mock, makedirs_mock, _uploader, _mock_game,
                                            _mock_subscriber):
        """
    ...
    def test_upload_http_error(self, module_patch, path_exists_mock, resolve_temp_file_mock, makedirs_mock, copy_mock,
                               make_archive_mock, gdrive_mock, datetime_mock, _uploader, _mock_game, _mock_subscriber,
                               http_error_mock):
        """
        ...
        progress_calls = [c for c in _mock_subscriber.call_args_list if c[0][0].type == c[0][0].type.Progress]
        ...
        done_event = _mock_subscriber.call_args_list[-1][0][0]
