...
def test_daemon_init_with_config(module_patch, path_exists_mock, json_config_holder_mock):
    """
    ...
    daemon = MockDaemon("TestService", requires_auth=False)
...
def test_daemon_init_without_config(module_patch, path_exists_mock):
    """
    ...
    daemon = MockDaemon("TestService", requires_auth=False, config_present=False)
...
def test_daemon_init_process_already_running(module_patch, path_exists_mock, mock_sys_exit):
    """
    ...
    MockDaemon("TestService", requires_auth=False)

    is_process_running.assert_called_once()
    mock_sys_exit.assert_called_once_with(0)


def test_daemon_start_work_loop_no_auth(mocker: MockerFixture, path_exists_mock, time_sleep_mock):
    """
...
def test_daemon_start_auth_required_delayed(mocker: MockerFixture, resolve_app_data_mock, path_exists_mock,
                                            module_patch, time_sleep_mock, logger_mock):
    """
    ...
    daemon = MockDaemon("AuthService", requires_auth=True)
...
def test_daemon_start_work_exception_handling(mocker: MockerFixture, resolve_app_data_mock, path_exists_mock,
    ...
    daemon = MockDaemon("ErrorService", requires_auth=False)
