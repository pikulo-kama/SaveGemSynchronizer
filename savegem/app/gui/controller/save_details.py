...
from savegem.app.gui.constants import QBool, QAttr
...
class SyncStatusController(WidgetController):
    ...
    def refresh(self, sync_status_badge: QCustomLabel):
        ...
        _logger.debug("game=%s, sync_status=%s", app().games.current.name, sync_status.name)

        # Only show badge when sync is not up-to-date with cloud.
        sync_status_badge.setProperty(QAttr.Hidden, QBool(sync_status == SyncStatus.UpToDate))
